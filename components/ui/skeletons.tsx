...
export function RevenueChartSkeleton() {
  return (
    <div className={`${shimmer} relative w-full overflow-hidden md:max-w-4xl mx-auto mb-6`}>
      <div className="mb-4 h-8 w-36 rounded-md bg-gray-100" />
      <div className="rounded-xl bg-gray-100 p-4">
        {/* Skeleton Chart Layout */}
        <div className="flex items-end gap-4 bg-white p-4 rounded-md h-[300px]">
          {/* Y-Axis Placeholder (Left) */}
          <div className="flex flex-col justify-between h-full text-sm text-gray-400">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="h-4 w-10 bg-gray-200 rounded-md"></div>
            ))}
          </div>

          {/* Bars Placeholder (Right) */}
          <div className="flex flex-1 items-end gap-2 md:gap-4 h-full">
            {[...Array(12)].map((_, i) => (
              <div key={i} className="flex flex-col items-center w-full">
                <div className="w-full bg-gray-200 rounded-md" style={{ height: `${Math.random() * 80 + 50}px` }} />
                <div className="mt-2 h-4 w-8 bg-gray-200 rounded-md" />
              </div>
            ))}
          </div>
        </div>
